...
def _jcs_utf16_sort_key(s: str) -> List[int]:
    """Return a sort key based on UTF-16 code unit values (RFC 8785 S3.2.3).
...
def _jcs_serialize_string(s: str) -> str:
    """Serialize a string per RFC 8785 S3.2.2.2.
...
def _jcs_serialize_number(value: Any) -> str:
    """Serialize a number per RFC 8785 S3.2.2.3.

    Follows the ECMAScript Number::toString algorithm
    (ECMA-262 S6.1.6.1.20): integers are rendered without a decimal
    point; floats use the shortest representation with
    ECMAScript-compatible exponential notation rules.
    """
    ...
    # ECMAScript formatting rules (ECMA-262 S6.1.6.1.20, steps 5-9)
    if k <= n <= 21:
        result = digits + "0" * (n - k)
    elif 0 < n <= 21:
...
def jcs_canonicalize(value: Any) -> str:
    ...
    Recursively processes dicts, lists, strings, numbers, booleans, and
    None.  Object keys are sorted by UTF-16 code-unit order; numbers
    follow the ECMAScript Number::toString algorithm; strings use
    JCS escape rules.
    """
    if value is None:
        return "null"
