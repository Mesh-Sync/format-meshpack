...
| `meshsync_thumbnails` | MeshSync | Thumbnail references (static, animated 360°, glTF preview) |
...
Thumbnail references generated by `worker-thumbnail-generation` pipeline. Maps thumbnail output types to resource hash references.
...
| Field | Description |
|-------|-------------|
| `static` | Reference to a static thumbnail image in resources/ (e.g., jpg, png) |
| `animated_360` | Reference to an animated 360° preview in resources/ (e.g., mp4, webm) |
| `glb_preview` | Reference to a glTF binary preview in resources/ (e.g., glb) |

> **Note**: Values use the format `{algo}:{hex_hash}.{ext}`, matching the resource naming convention. The `static` field typically corresponds to the entry's `preview_ref` field.
