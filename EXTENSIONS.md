...
All fields are hash-based resource references pointing to entries in the pack.

```json
"extensions": {
  "meshsync_thumbnails": {
    "v1": {
      "static": "sha256:abc123...",
      "animated_360": "sha256:def456...",
      "glb_preview": "sha256:789abc..."
    }
  }
}
```
...
| Field | Description |
|-------|-------------|
| `static` | Hash reference to a static thumbnail image (JPEG/PNG) |
| `animated_360` | Hash reference to an animated 360-degree preview (MP4/GIF) |
| `glb_preview` | Hash reference to a GLB 3D preview file |

> **Note**: All fields are optional. Values are content-addressable hash references
> (e.g., `sha256:...`, `sha512:...`, `blake3:...`) matching entries in the pack.
